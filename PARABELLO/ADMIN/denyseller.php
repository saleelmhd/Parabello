...
while($res=mysqli_fetch_array($query))
{
    ...
    if(isset($_REQUEST['btnDeny'.$count]))
    {
       ...
       mysqli_query($con,"UPDATE  parabello.seller_register SET status=0 WHERE no=".$res[0]) or die(mysqli_error($con));
       echo("<script>alert('Seller Denied')</script>");
...
?>
                    ...
                    <tr>
                        ...
                        <td>   <input type="submit" name="btnDeny<?php echo $count?>" class="btn btn-outline-info btn-block" value="Deny"></td>
