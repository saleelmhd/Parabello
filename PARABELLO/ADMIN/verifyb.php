...
$query=mysqli_query($con,"select * from parabello.buyer_register where status=0") or die(mysqli_error($con));
...
while($res=mysqli_fetch_array($query))
{
 ...
 {
    mysqli_query($con,"UPDATE  parabello.buyer_register SET status=1 WHERE no=".$res[0]) or die(mysqli_error($con));
    $uname=$res['name'].$res[0];
    $password=$res['mobile_no'].$res[0];
    $id=$res[0];
    mysqli_query($con,"INSERT INTO parabello.login(username,password,id,role)VALUES('$uname','$password',$id,'BUYER')") or die(mysqli_error($con));
    $subj = 'Verification';
    $to = $res['email'];
...
echo("<script>alert('Buyer verified')</script>");
 }
...
?>
                    ...
                    <tr>
                        ...
                        <td><input type="submit" name="btn<?php echo $count?>" class="btn btn-outline-info btn-block" value="verify"></td>
